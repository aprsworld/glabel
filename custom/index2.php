...
if ( $labelSize==1 ) {
	...
	if ( $print == true ) {
		for($i = 0;$i<$NumOfLines;$i++ ) {
			...
			$outToPrinter= $outToPrinter . str_replace('"','\"', trim($lines[$i]));
			$outToPrinter= $outToPrinter . "\"";
			$outToPrinter= $outToPrinter . "\n";
			$StartY+=$addSpace;
		}
...
//		echo "Printing Small Label";
	...
	if ( $print == true ) {
		for($i = 0;$i<$NumOfLines;$i++ ) {
			...
			$outToPrinter= $outToPrinter . ",N,\"";//N for normal, R for reversed
			$outToPrinter= $outToPrinter .str_replace('"','\"', trim($lines[$i]));//Data to be printed
			$outToPrinter= $outToPrinter . "\"";
			$outToPrinter= $outToPrinter . "\n";
			$StartX-=$addSpace+30;
		}
...
<body onload="makeprev()">
	...
	<div>
		...
		</form><button onclick="clearTextArea()" >Clear</button>
	</div>
	<h1>Preview</h1>	
	<div id="preview"></div>
